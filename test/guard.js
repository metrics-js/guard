...
tap.test(
	'Guard() - "permutationThreshold" is at 6 - one metric exceed this - should drop the metric from the stream when exceeding the threshold',
	(t) => {
		const guard = new Guard({
...
tap.test(
	'Guard() - "enabled" is "false" - metrics exceeds limits - should not interfere and should not store internal metrics',
	(t) => {
		const guard = new Guard({
			permutationThreshold: 6, // warns at 4
			metricsThreshold: 4,
...
tap.test(
	".getMetrics() - set metrics - call method - should return an Array with metric names without duplicates",
	(t) => {
		const guard = new Guard();
		const metrics = [
			new Metric({ name: "bar", description: "bar" }),
...
tap.test(
	".getLabels() - set metrics - call method - should return an Array with metric names without duplicates",
	(t) => {
		const guard = new Guard();
		const metrics = [
			new Metric({ name: "bar", description: "bar" }),
